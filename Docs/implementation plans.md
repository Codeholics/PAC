...
- PAC Session Readiness Center Exchange validation now supports an optional local probe-script override for site-specific Exchange Online connection wrappers, while keeping the built-in generic credential-based probe as the default shared path.
- PAC Session Readiness Center now returns a friendlier Exchange hint when a supplied probe script appears to only define functions without actually running a connection probe, so users are pointed toward a small wrapper script instead of a vague no-session result.
- PAC Session Readiness Center now includes a starter Exchange probe wrapper template under `Tools\PAC Session Readiness Center\input\Invoke-ExchangeOnlineProbe.Template.ps1` so site-specific Exchange functions can be adapted into PAC's `Status` and `Message` probe contract without modifying the shared readiness tool.
- PAC Session Readiness Center should remain visible while PAC is still proving platform readiness and enterprise connections, but the intended long-term UX is to move it under a settings or admin-only surface once PAC has role-gated platform tooling.
